...
You are the write agent. Your job: synthesize the analysis into a structured note and write it to the vault.

## Input
- Evidence ledger (from evidence agent)
- Rubric analysis (from analyze agent)
- `paper_type` → determines which template to use
- `citekey`, `item_key`, title, authors, year, venue, url
...
2. **Fill frontmatter** using the schema in `references/frontmatter.md`:
   - Required: type, source_type, title, citekey, item_key, authors, year, venue, url, pdf, status, paper_type, key_claims, created, updated, tags
   - `pdf` field: `[[raw/<citekey>/<citekey>.md]]`
...
3. **Fill the body sections** from the template. For each section:
   ...
   - Citation format (portable, no private syntax):
     ...
     - Figures: `![[raw/<citekey>/assets/fig_xxx.png]]` + 1-3 sentence caption
     - Equations: `$LaTeX# Phase 4: Write Agent
...
3. **Fill the body sections** from the template. For each section:
   - Use evidence from the ledger — every claim needs a source reference.
 (p.N)
     - Cross-ref: `[[other_citekey]]`
     - **Zotero annotation link** (optional, if annotation_key was recorded in Phase 2): after the page ref, add `[annotation](zotero://select/library/items/ANNOTATIONKEY)` — makes the reference clickable in Zotero desktop. Example: `> we propose... (p.3, [annotation](zotero://select/library/items/ABCD1234))`
